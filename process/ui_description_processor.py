"""
Processor for description-ui-en.md files.

Parses UI description documentation into graph entities and relationships.
"""
...
class UIDescriptionProcessor:
    """Parse UI description markdown into graph entities + relationships."""
    ...
    def process(self) -> Dict[str, Any]:
        ...
        if not self.file_path.exists():
            raise FileNotFoundError(f"UI description file not found: {self.file_path}")
...
def main() -> None:
    ...
    for module in modules:
        output_path = f"/data_hdd_16t/vuongchu/nxGREG/json/{module}/{module}-ui-description.json"
        
        # contract-list has no basic tab, so basic_tab_name will be auto-set in __init__
        processor = UIDescriptionProcessor(base_path, module_name=module)
        processor.save_to_json(output_path)
        print(f"Processed {module} UI description -> {output_path}")
    
    print("UI description JSON files generated successfully!")
